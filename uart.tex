...
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{fskToUART.png}
...
A l'état 1, une T2 est attendue. Si une T2 est reçue, alors le message a été entièrement reçu, l'état retourne à zéro et le récepteur renvoie séparément \ilcode{o<1-0>} et \ilcode{p<7-0>} à partir de T2\footnote{On voit donc ici pourquoi il est plus intéressant d'envoyer l'ordre dans T2: cela permet de ne pas devoir le stocker à l'état 0.} et de \ilcode{p<7-4>} en mémoire. Sinon, il y a eu une erreur, l'état retourne quand même à zéro et l'UART propulsion demande que le message entier soit répété. Ceci est exécuté par le code \ref{src:propReceptor}.
\begin{listing}[htbp]
\begin{minted}[
frame=lines,
framesep=1mm,
...
\end{listing}
Une trame \ilcode{0x01} demande donc la répétition du message. Il s'agit donc aussi de la seule trame que le récepteur communication peut doit gérer. Ce récepteur est donc lui aussi très simple. Il est présenté dans la section suivante.
...
\end{listing}
Le cas où la trame reçue côté communication n'est pas valide n'est pas pris en compte, et ne s'est jamais présenté. Tous les scénarios possibles ont donc été examinés et le code présenté implémente donc une communication UART complète et parfaitement fonctionnelle. Le traitement de l'ordre et du paramètre doit maintenant être abordé. Ce traitement est appelé dans la fonction \ilcode{handleParam2} du code source \ref{src:propReceptor}, et va être présenté dans le chapitre suivant.
